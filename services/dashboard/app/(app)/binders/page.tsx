        ...
        <div className="grid grid-2" style={{ marginBottom: 16 }}>
          ...
          <div className="card">
            ...
            <p className="body-text" style={{ marginTop: 10 }}>
              A binder is built from the immutable event ledger and the reasoning records, by a
              template. Not by a model — the renderer has no path to one, which is asserted by a
              test rather than promised, because an import graph is the only form of <em>no model
              was involved</em> that cannot quietly stop being true.
            </p>
            <p className="card-note" style={{ marginTop: 12 }}>
              Every closed review below has one. It downloads as a single self-contained HTML file
              with a print stylesheet, so it can be attached to an audit response as it is.
            </p>
          </div>
        </div>
                ...
                <tbody>
                  {shown.map((b) => (
                    <tr key={b.review_id}>
                      ...
                      <td className="right">
                        <a href={`/api/binders/${b.review_id}`} style={{ fontWeight: 600 }}>
                          Download
                        </a>
                      </td>
                    </tr>
