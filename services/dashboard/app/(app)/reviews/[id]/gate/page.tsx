...
import ApproveGate from "./approve";
import { CAN_APPROVE, may } from "../../../../../lib/auth";
import {
  BandPill,
  Empty,
  FixtureBanner,
...
export default async function GateCard({ params }: { params: Promise<{ id: string }> }) {
  ...
  let found: Doc | null;
  try {
    found = await loadReview(orgId, id);
  } catch (error) {
    return (
      <>
  ...
  const [result, raised, memo, tokens] = await Promise.all([
    scoreFor(orgId, id),
    findingsFor(orgId, id),
    memoFor(orgId, id),
    approvalsFor(orgId, id),
  ]);
      ...
      <div className="scroll-area" style={{ animation: "db-rise 260ms ease both" }}>
        ...
        <div className="grid grid-2">
          ...
          <div className="stack">
            ...
            {waiting ? (
              <div className="card">
                ...
                <div style={{ marginTop: 12 }}>
                  <ApproveGate
                    reviewId={id}
                    scope={(scope as "contact" | "decision") || "decision"}
                    email={principal.email}
                    canApprove={may(principal.role, CAN_APPROVE)}
                  />
                </div>
              </div>
            ) : null}
            ...
            <ReadOnlyNote>
              <strong style={{ color: "var(--ink)" }}>This console holds no signing key.</strong>{" "}
              Your approval is signed by a separate service that verifies you independently — it
              does not take this page&rsquo;s word for who you are or what you may do. The token it
              produces is{" "}
              <strong style={{ color: "var(--ink)" }}>single-use and scoped to this review and
              this gate</strong>: an approval given for first contact cannot be replayed to accept
              risk, and neither can be used twice.
            </ReadOnlyNote>
