...
export default async function VendorDetail({
  ...
  const [score, memo, findings, qa, docs, subs, dossier, feed, tl] = await Promise.all([
    reviewId ? scoreFor(reviewId) : null,
    reviewId ? memoFor(reviewId) : "",
    reviewId ? findingsFor(reviewId) : [],
    reviewId ? answers(reviewId) : [],
    reviewId ? screenings(reviewId) : [],
    chain(id),
    vendorDossier(id),
    signals(),
    reviewId ? timeline(reviewId) : [],
  ]);
    ...
    <>
      ...
      <div className="scroll-area" style={{ animation: "db-rise 260ms ease both" }}>
        ...
        {!latest ? (
          <div className="card">
            <Empty>
              No review has been opened for this vendor. Open one with{" "}
              <code className="mono">make open-review VENDOR={id}</code>.
            </Empty>
          </div>
        ) : tab === "Overview" ? (
          ...
          <div className="grid grid-2">
            ...
            <div className="stack">
              ...
              <ReadOnlyNote>
                This console cannot approve anything. It holds no signing key and no write path —
                if a surface every reviewer can reach could mint an approval, the gateway refusing
                to sign would be decorative. Release a gate with{" "}
                <code className="mono">python -m scripts.issue_token --review {reviewId} --scope decision</code>.
              </ReadOnlyNote>
            </div>
          </div>
        ) : (
          <div className="stack">
            <div className="card">
              ...
              <p className="card-note" style={{ marginTop: 6 }}>
                {tl.length} entries. The full binder is rendered by a template, never by a model:{" "}
                <code className="mono">make binder REVIEW={reviewId}</code>.
              </p>
              <div className="timeline" style={{ marginTop: 12 }}>
                {tl.slice(-25).map((e, i) => (
