...
export default async function Overview() {
  ...
  let feed: Doc[];
  try {
    [data, feed] = await Promise.all([overview(), signals()]);
  } catch (error) {
    return (
      <>
  ...
  return (
    <>
      <PageHead
        crumb="Overview"
        title="Overview"
        subtitle="Vendor security posture across the portfolio, and what the fleet needs from you today."
      />
          ...
          <div className="card card-tight">
            ...
            {feed.length === 0 ? (
              <div style={{ padding: "0 22px 18px" }}>
                <p className="empty">
                  No signals recorded. The Watchdog sweeps on a schedule; run{" "}
                  <code className="mono">make demo-fixtures</code> to produce some.
                </p>
              </div>
            ) : (
