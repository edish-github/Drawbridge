...
while IFS=$'\t' read -r name bucket role; do
  ...
  gcloud --project="${PROJECT_ID}" storage buckets add-iam-policy-binding \
...
while IFS=$'\t' read -r name secret; do
  [[ -z "${name}" ]] && continue
  if ! gcloud --project="${PROJECT_ID}" secrets describe "${secret}" >/dev/null 2>&1; then
    log "  secret ${secret} does not exist yet; create it before granting access"
    continue
  fi
  log "  ${name} -> secret ${secret}"
  gcloud --project="${PROJECT_ID}" secrets add-iam-policy-binding "${secret}" \
    --member="serviceAccount:$(sa_email "${name}")" \
